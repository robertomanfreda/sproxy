...
When there is no protocol Sproxy will try to proxy the request, to the requested URL, using HTTPS first;  
...
run it out of the docker context you are free to do it but I'll not provide any information about configuration or other
stuff.
...
services:
  sproxy:
    ...
    image: robertoman/sproxy:2.0.3-SNAPSHOT
    ports:
      - 6380:6380
    volumes:
      - ./config:/config
...
So after this you can simply run `docker-compose up -d` (at the same level of docker-compose.yaml) file to start Sproxy.
...
- `docker run -d --name sproxy -p 6380:6380 com.robertoman/sproxy:custom`