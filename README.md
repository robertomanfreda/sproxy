...
By default sproxy try to register itself on eureka, if you don't need it explicitly set the `config.eureka.enabled` 
property your application.yaml to false.  
    
If you need to enable thediscovery client feature set the `config.eureka.enabled` property in your application.yaml 
to `true` then add your `bootstrap.yaml`configuration file in the config folder (explained below).  
Something like this (this is an example you need to adapt the file according to your needs):  
```yaml
spring:
  application:
    name: sproxy
...
- sproxy
     ...
     |            |
     |            +file--> bootstrap.yaml    #(if you need eureka and the `config.eureka.enabled` property == true)
     |            |
